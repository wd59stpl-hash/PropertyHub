import React, { useState } from 'react';
import { useForm } from 'react-hook-form';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate, Link } from 'react-router-dom';
import { Mail, Lock, User, ArrowRight, Building2, ShoppingBag } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { motion } from 'framer-motion';
import { registerUser } from '../../redux/slices/authSlice';

const Register = () => {
    const [role, setRole] = useState('buyer'); 
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { loading } = useSelector((state) => state.auth);

    const { register, handleSubmit, formState: { errors } } = useForm();

    const onSubmit = (data) => {
        dispatch(registerUser({ ...data, role })).then((res) => {
            if (!res.error) {
                toast.success("Account created! Please verify your email.");
                navigate('/verify-email', { state: { email: data.email } });
            } else {
                toast.error(res.payload || "Registration failed");
            }
        });
    };

    return (
        <div className="min-h-screen w-full flex bg-[#F8F9FB] font-sans">            
            <div className="hidden lg:flex w-[55%] relative overflow-hidden bg-[#080E4B]">
                <img 
                    src="https://images.unsplash.com/photo-1512917774080-9991f1c4c750?q=80&w=2070&auto=format&fit=crop" 
                    className="absolute inset-0 w-full h-full object-cover opacity-50 scale-110 hover:scale-100 transition-transform duration-[10s]" 
                    alt="Luxury Mansion" 
                />                
                <div className="absolute inset-0 bg-gradient-to-tr from-[#080E4B] via-transparent to-transparent opacity-90" />
                <div className="relative z-10 w-full flex flex-col justify-between p-20 text-white">
                    <motion.div 
                        initial={{ opacity: 0, y: -20 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="flex items-center gap-3"
                    >
                        <div className="w-12 h-12 border-2 border-[#C5A358] rounded-full flex items-center justify-center">
                            <div className="w-8 h-8 bg-[#C5A358] rounded-full" />
                        </div>
                        <span className="text-2xl font-serif tracking-[0.2em] uppercase">PropertyHub</span>
                    </motion.div>

                    <div className="space-y-8">
                        <motion.div
                            initial={{ opacity: 0, x: -50 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ delay: 0.3 }}
                        >
                            <span className="text-[#C5A358] font-bold tracking-[0.4em] text-xs uppercase mb-4 block">New Membership</span>
                            <h1 className="text-7xl font-serif leading-tight">Start Your <br /> <span className="italic">Legacy.</span></h1>
                        </motion.div>

                        <div className="flex gap-12">
                            <div className="bg-white/5 backdrop-blur-md p-6 border border-white/10 rounded-sm">
                                <p className="text-sm font-light tracking-widest text-white/80">"The best investment on earth is earth."</p>
                            </div>
                        </div>
                    </div>

                    <p className="text-white/40 text-xs tracking-widest uppercase">© 2024 PropertyHub Luxury Real Estate</p>
                </div>
            </div>

            <div className="flex-1 flex items-center justify-center p-6 sm:p-12 lg:p-20 bg-white">
                <motion.div 
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="w-full max-w-md"
                >
                    <div className="mb-10 text-center lg:text-left">
                        <h2 className="text-4xl font-serif text-[#080E4B] mb-3">Create Account</h2>
                        <p className="text-gray-400 font-medium">Join our exclusive community of property owners.</p>
                    </div>

                    <div className="flex mb-10 border-b border-gray-100">
                        <button 
                            onClick={() => setRole('buyer')}
                            className={`flex-1 pb-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all ${role === 'buyer' ? 'text-[#080E4B] border-b-2 border-[#C5A358]' : 'text-gray-300'}`}
                        >
                            Become a Buyer
                        </button>
                        <button 
                            onClick={() => setRole('seller')}
                            className={`flex-1 pb-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all ${role === 'seller' ? 'text-[#080E4B] border-b-2 border-[#C5A358]' : 'text-gray-300'}`}
                        >
                            Become a Seller
                        </button>
                    </div>

                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        <div className="relative group">
                            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C5A358] mb-2 block">Full Name</label>
                            <div className="flex items-center border-b-2 border-gray-100 group-focus-within:border-[#080E4B] transition-all py-3">
                                <User className="text-gray-300 group-focus-within:text-[#C5A358] transition-colors" size={20} />
                                <input
                                    type="text"
                                    {...register("name", { required: "Name is required" })}
                                    className="w-full bg-transparent px-4 outline-none text-gray-700 font-medium placeholder:text-gray-200"
                                    placeholder="Enter your full name"
                                />
                            </div>
                            {errors.name && <p className="text-[10px] text-red-500 font-bold mt-2 uppercase">{errors.name.message}</p>}
                        </div>

                        <div className="relative group">
                            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C5A358] mb-2 block">Email Address</label>
                            <div className="flex items-center border-b-2 border-gray-100 group-focus-within:border-[#080E4B] transition-all py-3">
                                <Mail className="text-gray-300 group-focus-within:text-[#C5A358] transition-colors" size={20} />
                                <input
                                    type="email"
                                    {...register("email", { required: "Email is required" })}
                                    className="w-full bg-transparent px-4 outline-none text-gray-700 font-medium placeholder:text-gray-200"
                                    placeholder="yourname@luxury.com"
                                />
                            </div>
                            {errors.email && <p className="text-[10px] text-red-500 font-bold mt-2 uppercase">{errors.email.message}</p>}
                        </div>

                        <div className="relative group">
                            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C5A358] mb-2 block">Create Password</label>
                            <div className="flex items-center border-b-2 border-gray-100 group-focus-within:border-[#080E4B] transition-all py-3">
                                <Lock className="text-gray-300 group-focus-within:text-[#C5A358] transition-colors" size={20} />
                                <input
                                    type="password"
                                    {...register("password", { required: "Password is required", minLength: { value: 6, message: "Min 6 characters" } })}
                                    className="w-full bg-transparent px-4 outline-none text-gray-700 font-medium placeholder:text-gray-200"
                                    placeholder="••••••••••••"
                                />
                            </div>
                            {errors.password && <p className="text-[10px] text-red-500 font-bold mt-2 uppercase">{errors.password.message}</p>}
                        </div>

                        <div className="pt-6">
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full h-16 bg-[#080E4B] text-white flex items-center justify-center gap-4 group relative overflow-hidden transition-all active:scale-95"
                            >
                                <span className="relative z-10 font-black text-xs uppercase tracking-[0.3em]">
                                    {loading ? "Processing..." : `Register as ${role}`}
                                </span>
                                {!loading && <ArrowRight size={18} className="relative z-10 group-hover:translate-x-2 transition-transform" />}
                                <div className="absolute inset-0 bg-[#C5A358] translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
                            </button>
                        </div>
                    </form>
                    <div className="mt-10 text-center">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                            Already part of the elite? 
                            <Link to="/login" className="ml-2 text-[#080E4B] border-b border-[#C5A358] pb-1 hover:text-[#C5A358] transition-colors">
                                Log In Here
                            </Link>
                        </p>
                    </div>
                </motion.div>
            </div>
        </div>
    );
};

export default Register;